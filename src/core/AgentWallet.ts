...
const ASSET_ADDRESSES: Record<string, AssetAddresses> = {
  mainnet: {
...
export class AgentWallet {
  ...
  private assetAddresses: AssetAddresses;
  private aggrClient?: AGGRClient;
  private activeOrders: Map<string, () => void>;
  private customAssets: Map<string, CustomAssetAddresses>;
...
if (config.privateKey) {
  this.radixClient = new RadixClient(config.privateKey, this.networkConfig);
} else {
...
getAssetAddress(asset: SupportedAsset): string {
  ...
  if (custom) {
    const address = custom[this.config.network as keyof CustomAssetAddresses];
    if (address) return address;
    throw new Error(
      `[AgentWallet] Asset ${asset} has no address for network ${this.config.network}`
    );
  }

  // Si no encuentra busca en los assets por defecto
  const defaults = ASSET_ADDRESSES[this.config.network];
  const address = defaults[asset as keyof AssetAddresses];
  if (!address) {
    throw new Error(
      `[AgentWallet] Asset ${asset} not found. Use registerAsset() to add custom assets.`
    );
  }
  return address;
}
...
registerAsset(symbol: string, addresses: CustomAssetAddresses): void {
  const upper = symbol.toUpperCase();
  if (ASSET_ADDRESSES[this.config.network][upper as keyof AssetAddresses]) {
    throw new Error(
      `[AgentWallet] Asset ${upper} already exists as a default asset.`
    );
  }
